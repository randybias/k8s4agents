# Cluster Assessment Capability Specification
...
The assessment collects comprehensive data but avoids:
- Excessive API calls that could impact cluster performance
- Streaming watches (uses snapshot approach)
- Deep recursive queries (stays at resource level)
- Collection of sensitive data (secrets, credentials)

### Report Format Rationale

Markdown format chosen for:
- Human readability
- Version control friendly (git diff)
- Easy sharing and collaboration
- Wide tool support (rendering, conversion)
- Embedding in documentation systems

### Health Score Calibration

Health scoring is informational and relative:
- Not a compliance standard
- Helps track improvement over time
- Highlights areas needing attention
- Should be customized for organizational needs

### Extensibility

The assessment framework is designed for extension:
- Additional data collectors can be added as functions
- Report sections can be expanded
- Organization-specific checks can be integrated
- Custom scoring logic can be applied
